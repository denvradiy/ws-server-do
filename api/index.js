...
const msg_BROADCAST_INTERVAL = 500;
...
let messageIndex = 0; // Keep track of which msg to send next
...
wss.on('connection', (ws) => {
  ...
  // If mswa broadcasting is active, start sending msgs to the new client
  if (isBroadcastingMswa) {
    sendNextmsg(ws); // Send next msg to the new client immediately
  }
...
// Send the next message from the array to a specific client
const sendNextmsg = (ws) => {
  if (ws.readyState === OPEN) {
    ws.send(JSON.stringify({mswa: messages[messageIndex]}));
  }
  messageIndex = (messageIndex + 1) % messages.length; // Loop back to start after the last msg
};

// Broadcast mswa array if broadcasting is enabled
setInterval(() => {
  if (isBroadcastingMswa && messages.length > 0) {
    // Send the next msg from the array to all clients
